...
from app.database.database import get_member
import time as t
...
class RankSystem(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        modules = [basename(f)[:-3] for f in filenames if isfile(f) and not f.endswith('__init__.py') and not f.endswith('baserank.py')]
    ...
    async def handle_rank_up(self, member: GuildMember, level: int):
        if member != None:
